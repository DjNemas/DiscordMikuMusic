...
{
    public class LibSodiumUpdaterService : ILibSodiumUpdaterService
    {
        private const string _packageName = "libsodium";
        private const string _releaseInfoFile = "libsodium_release.json";
        ...
        private readonly ILogger<LibSodiumUpdaterService> _logger;
        private readonly AppInfo _appInfo;
        private NuGetPackageInfoDTO? _localPackageInfo;

        public LibSodiumUpdaterService(ILogger<LibSodiumUpdaterService> logger, AppInfo appInfo)
            : this(logger, appInfo, Path.GetDirectoryName(AppContext.BaseDirectory)!) { }

        public LibSodiumUpdaterService(ILogger<LibSodiumUpdaterService> logger, AppInfo appInfo, string downloadPath)
        {
            _logger = logger;
            _appInfo = appInfo;
        ...
        {
            _logger.LogInformation("Starting libsodium update check...");
            ...
            if (!success)
            {
                _logger.LogError("Update to libsodium {Version} failed.", latestInfo.Version);
                return null;
            }

            await SaveReleaseInfoToFile(latestInfo);
            _logger.LogInformation("libsodium successfully updated to version {Version}.", latestInfo.Version);
            return latestInfo;
        }
        ...
        private async Task<string?> FetchLatestVersion()
        {
            ...
            _logger.LogDebug("Fetching latest libsodium version from NuGet at {Url}", indexUrl);
            ...
            _logger.LogDebug("Latest libsodium NuGet version: {Version}", latest);
            return latest;
        }
        ...
        private bool IsUpdateNeeded(NuGetPackageInfoDTO latestInfo)
        {
            ...
            _logger.LogInformation("libsodium is already up to date (version: {Version}).", localInfo.Version);
            return false;
        }
        ...
        private async Task<bool> DownloadAndExtract(NuGetPackageInfoDTO info)
        {
            ...
            _logger.LogInformation("Downloading libsodium {Version} from {Url}", info.Version, downloadUrl);
            ...
            {
                _logger.LogError(ex, "Failed to download or extract libsodium from NuGet package");
                return false;
            }
        }
        ...
        private async Task<bool> ExtractDllFromNupkg(Stream nupkgStream)
        {
            ...
            var dllEntry = archive.Entries.FirstOrDefault(e =>
                ...
                e.Name.Equals("libsodium.dll", StringComparison.OrdinalIgnoreCase));

            if (dllEntry is null)
            {
                _logger.LogError("libsodium.dll not found in runtimes/win-x64/native/ of the NuGet package.");
                return false;
            }

            _logger.LogDebug("Found {Entry} in NuGet package.", dllEntry.FullName);

            var filePath = Path.Combine(_runningAssemblyFolder, "libsodium.dll");
            ...
            _logger.LogInformation("libsodium.dll written to {FilePath}", filePath);
            return true;
        }

        private async Task SaveReleaseInfoToFile(NuGetPackageInfoDTO info)
        {
            _logger.LogInformation("Update to libsodium {Version} succeeded. Saving release info locally.", info.Version);
        ...
        private NuGetPackageInfoDTO? ReadInfosFromFile()
        {
            ...
            if (info is null)
                ...
                _logger.LogInformation("Loaded local libsodium release info: Version={Version}", info.Version);
